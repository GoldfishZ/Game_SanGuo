...
def test_weakening_chain_reduces_the_specified_enemy_without_chain_allies():
    guo_huanghou = get_general_by_name("郭皇后")
    enemies = [
    ...
    result = team.use_skill(guo_huanghou, [enemies[2]], BattleContext(team, enemy_team))
    ...
    assert enemies[0].get_effective_force() == 8
    assert enemies[1].get_effective_force() == 6
    assert enemies[2].get_effective_force() == 1
...
def test_weakening_chain_adds_targets_for_multiple_chain_generals():
    ...
    assert enemies[1].get_effective_force() == 7
    assert enemies[2].get_effective_force() == 2


if __name__ == "__main__":
    test_guo_huanghou_data_and_skill()
    test_weakening_chain_reduces_the_specified_enemy_without_chain_allies()
